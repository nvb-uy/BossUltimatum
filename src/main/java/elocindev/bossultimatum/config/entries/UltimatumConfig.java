...
public class UltimatumConfig {
    ...
    public class Ultimatum {
        ...
        public boolean direct_kill;
        public int indirect_radius;
        public int minimum_hp;
        public List<InnerHealingConfig> healing_events;

        public Ultimatum(String entity_regex, boolean direct_kill, int indirect_radius, int minimum_hp, List<InnerHealingConfig> healing_events) {
            this.entity_regex = entity_regex;
            this.direct_kill = direct_kill;
    ...
    @Comment("   - direct_kill: If true, the event will only execute on direct kill from the boss. If false, it will instead search for nearby bosses in a radius. Performance may be impacted when false.")
    @Comment("   - indirect_radius: If direct_kill is false, this is the radius to search for nearby bosses.")
    ...
    @Comment("   - ultimatum_death_count: The amount of times the boss must die before it starts performing the ultimatum event. This currently only works in the fabric version, if you are on NeoForge, please use the mod with Connector to get this feature.")
    ...
    public List<Ultimatum> ultimatums = List.of(
        new Ultimatum("example_boss_mod:.*", true, 64, 0, List.of(
